...
$$
 \Epsilon_t = \sum_{k=1}^n \epsilon_k = \sum_{k=1}^n \left(\log_2(m^*_k) + \log_2\left(\frac{60}{t \cdot b}\right) - a_k\right)^2,  \quad m^* \in \mathbb{N}
$$
...
$$
Then, we can rewrite $\Epsilon$ as:
$$
\Epsilon_t = \sum_{k=1}^n \epsilon_k = \sum_{k=1}^n \left(\log_2(m^*_k) + u(t) - a_k\right)^2, \quad m^* \in \mathbb{N}
$$
...
$$
\Epsilon_t=\sum_{k=1}^n\epsilon_k = \left(\log_2(\hat{m}_k^*) + u(t) - a_k\right)^2
$$


### Optimizing Over Tempi
To find the optimal tempo $t$, we iterate over all possible tempi $t \in T$, and for each tempo, compute the corresponding $\hat{m}_k^*$ values and evaluate the total error $\Epsilon_t$. The optimal tempo $t^*$ is the one that yields the smallest error:
$$
t^* = \arg\min_{t \in T} \Epsilon_t
$$
