...
Inside the darkroom view, sound mode shows the same plan as the other two: the patch numbers and the seconds each patch ends at. Beside each of them is a bar as long as the wait for that patch, and when a run is going the bars fill in turn, each over exactly its own wait. They are all drawn to one speed, so the strip is a picture of its own timing — the first bar is the long one, the base exposure, and the rest grow by a step each — and whichever bar is filling is where the run has got to. Sound and picture come off the same plan and are set going together, so nothing has to be kept in step with anything.
